...
export const ACTIVE_TELEGRAM_BOT_TOKEN = '8828063101:AAE8U4h1ITOku7m0w6mid_X08B4Sn_DaOaA';
...
async function initOrRestartTelegramBot(rawToken?: string | null) {
  ...
  try {
    ...
    const sendMainMenu = async (chatId: number, customText?: string) => {
      ...
      const text =
        customText ||
        `🏛 <b>1-Sektor Murojaatlar rasmiy Telegram botiga</b> xush kelibsiz.\n\n` +
    ...
    const handleShowOrgs = (chatId: number) => {
      let text = `🏢 <b>1-Sektor tashkilotlari va biriktirilgan mas'ul xodimlar:</b>\n\n`;
    ...
    const submitAppealFromSession = async (chatId: number, session: UserSessionData) => {
      ...
      const newAppeal: Appeal = {
        ...
        address: session.address || session.selectedMfy || '1-Sektor hududi',
      ...
      if (session.step === 'WAITING_CONTENT') {
        ...
        await telegramBot?.sendMessage(
          ...
          `✍️ <b>Murojaat matni qabul qilindi!</b>\n\n` +
            ...
            `Agar barchasi tayyor bo‘lsa, murojaatni 1-Sektor shtabiga va mas'ul tashkilotga yo‘llash uchun pastdagi <b>«📤 Yuborish»</b> tugmasini bosing:`,
          {
            parse_mode: 'HTML',
            reply_markup: {
              keyboard: [
...
app.post('/api/appeals', (req, res) => {
  ...
  const newAppeal: Appeal = {
    ...
    address: address || (mahalla ? `${mahalla}` : '1-Sektor hududi'),
    content,
    attachmentUrl,
